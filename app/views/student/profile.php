...
<body>
  <div class="container mt-5 profile">
    <form action="<?= BASEURL ?>/Profile/<?= isset($_GET['npm']) ? "AddFriend" : "UpdateStudent" ?>" enctype="multipart/form-data" method="POST">
      <div class="row d-flex justify-content-center">
        <div class="d-flex flex-row align-items-center mt-4 ms-5">
          ...
          <?php if (!isset($_GET['npm'])) : ?>
            <?php if (!isset($_GET['update'])) : ?>
              <a href="<?= BASEURL ?>/Profile/Student?update=1" class="ms-auto me-5 btn btn-save">Update Profile</a>
            <?php elseif (isset($_GET['update'])) : ?>
              <a href="<?= BASEURL ?>/Profile/Student" class="ms-auto me-5 btn btn-save">Cancel</a>
            <?php endif; ?>          
          <?php endif; ?>
        </div>
        ...
        <div class="col-md-8">
          <div class="p-3 py-5">
            ...
            <div class="row mt-2">
              <div class="col-md-5">
                <input type="hidden" name="profile_image" class="form-control" placeholder="Image" value="<?= $data['image'] ?>">
                <input type="hidden" name="npm" class="form-control" placeholder="NPM" value="<?= $data['npm'] ?>">
                <?php if (isset($_GET['npm'])) : ?>
                <input type="hidden" name="relating_student" class="form-control" placeholder="Myself" value="<?= $_COOKIE['npm'] ?>">
                <input type="hidden" name="related_student" class="form-control" placeholder="Friends" value="<?= $data['npm'] ?>">
                <?php endif; ?>
            ...
            <div class="row mt-3">
              <div class="col-md-2">
                <label for="social_score">Social Score</label>
                <input type="text" name="social_score" class="form-control" placeholder="Social Score" value="<?= $data['social_score'] ?>" disabled>
              </div>
              <div class="col-md-10">
                <?php if (isset($_GET['update'])) : ?>
                ...
                <?php endif; ?>       
                <?php if(!in_array($_COOKIE['npm'], $data['friends'])) : ?>
                  <?php if (isset($_GET['npm'])) : ?>
                    <div class="mt-4 text-right d-grid col-md-10">
                      <button class="btn btn-save profile-button" type="submit" name="submit" <?= in_array($_COOKIE['npm'], $data['pending_friends']) ? "disabled" : "" ?> >Add Friends</button>
                    </div>
                  <?php endif; ?>            
                <?php endif; ?>            
              </div>
